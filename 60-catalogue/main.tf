...
# Terraform copies files to catalogue server
    provisioner "file"{
        source = "catalogue.sh"
...
resource "aws_lb_target_group" "catalogue" {
  ...
  protocol = "HTTP"
  vpc_id   = local.vpc_id
  deregistration_delay = 60 # 	waiting period before deleting the instances
...
resource "aws_launch_template" "catalogue" {
  ...
  # tags attched to the instances	
   tag_specifications {
    ...
    # tags attched to the volume created by instance
    tag_specifications {
...
resource "aws_autoscaling_group" "catalogue" {
  ...
  target_group_arns = [aws_lb_target_group.catague.arn]
  dynamic "tag" { # we will get the iterator with name as tag
	for_each =  merge(
		local.common_tags,
		{
			Name = "${local.common_name_suffix}-catague"
		}
	)
	content {
		key                 = tag.key
